...
export default function DashboardPage() {
  ...
  useEffect(() => {
    if (!business) return
    const interval = setInterval(() => loadNotifications(business.id), 15000)
  ...
  const loadDashboard = async (phoneNum: string) => {
    ...
    await loadNotifications(businessData.id)
    setLoading(false)
  }
  ...
  const markAllRead = async () => {
    if (!business) return
    await supabase.from('notifications').update({ read: true }).eq('business_id', business.id).eq('read', false)
    setNotifications([])
  }
        ...
        {activeTab === 'orders' && stats && (
          <>
            ...
            {notifications.length > 0 && (
              <div className="bg-gradient-to-r from-green-500 to-emerald-600 rounded-2xl p-4 mb-4 shadow-lg shadow-green-500/30">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <span className="text-2xl">🔔</span>
                    <div>
                      <p className="text-white font-bold text-sm">{notifications.length} new payment{notifications.length > 1 ? "s" : ""}!</p>
                      <p className="text-white/80 text-xs">Tap to send receipt</p>
                    </div>
                  </div>
                  <button onClick={markAllRead} className="text-white/80 text-xs hover:text-white px-2 py-1">
                    Dismiss all
                  </button>
                </div>
                <div className="space-y-2">
                  {notifications.slice(0, 3).map(n => (
                    <div key={n.id} className="bg-white/15 backdrop-blur-sm rounded-xl p-3 flex items-center justify-between">
                      <div className="flex-1 min-w-0">
                        <p className="text-white font-semibold text-sm truncate">{n.message}</p>
                        <p className="text-white/80 text-xs">{new Date(n.created_at).toLocaleTimeString("en-IN", { hour: "numeric", minute: "2-digit", hour12: true })}</p>
                      </div>
                      <Link 
                        href="/orders" 
                        onClick={() => markAsRead(n.id)} 
                        className="bg-white text-green-600 px-3 py-1.5 rounded-lg text-xs font-bold ml-2 whitespace-nowrap hover:bg-green-50 transition-colors"
                      >
                        Send Receipt
                      </Link>
                    </div>
            ...
            <div className="bg-white rounded-3xl p-6 text-gray-900 border border-gray-200 shadow-lg mb-4 relative overflow-hidden">
              <div className="relative z-10">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-2xl">💰</span>
                  <p className="text-gray-600 text-sm font-medium">{getRangeLabel(dateRange)} · Revenue</p>
                </div>
                <h2 className="text-5xl font-bold mb-2 tracking-tight text-gray-900">Rs.{stats.totalCollected.toLocaleString('en-IN')}</h2>
                <p className="text-gray-500 text-sm mb-4">Avg order: Rs.{stats.avgOrderValue}</p>
